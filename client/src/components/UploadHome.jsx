...
import MisaLogo from "../images/Logo_MISA.svg";
...
function UploadHome() {
  ...
  const handleOk = async () => {
    ...
    console.log(file.originFileObj.name);
    if (isAuthenticated && user.role === "USER") {
      await axios({
        url: "https://localhost:44327/api/Invoice/importEnvoice",
        method: "POST",
        ...
        if (res.data.status === true) {
          let filename = file.originFileObj.name;
          let a = filename.replace(/\.xml$/, "");
          setConvertedFileUrl(res.data.data);
          setConvertedFile(a);
          setState("success");
          messageApi.open({
            type: "success",
            content: "Convert and save history successfully",
          });
  ...
  const onClickHandler = async () => {
    if (file === null) {
      ...
      messageApi.open({
        type: "error",
        content: "File upload error ! Please upload again!",
      });
      return;
    } else {
          ...
          >
            <form>
              <div className="container bg-slate-100 p-5 rounded-lg">
                ...
                <a
                  class="mx-3 font-bold text-green-600 bg-lime-100"
                  href={convertedFileUrl}
                  download={convertedFile}
                  target="_blank"
                >
                  <span title={convertedFile}>{convertedFile}</span>
                  <span class="ml-4">
                    <DownloadOutlined />
                  </span>
                </a>
